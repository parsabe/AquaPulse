...
class LiveCameraViewerScreen extends ConsumerStatefulWidget {
  const LiveCameraViewerScreen({Key? key}) : super(key: key);

  @override
  ConsumerState<LiveCameraViewerScreen> createState() => _LiveCameraViewerScreenState();
}
...
class _LiveCameraViewerScreenState extends ConsumerState<LiveCameraViewerScreen>
                ...
                children: [
                  Container(
                    ...
                    decoration: const BoxDecoration(
                      ...
                      boxShadow: [
                        BoxShadow(color: AppTheme.cyanAccent, blurRadius: 8, spreadRadius: 2),
                      ],
                    ),
                  ),
                      ...
                      children: [
                        ...
                        Positioned(
                          ...
                          child: Container(
                            ...
                            child: Row(
                              children: [
                                const Icon(Icons.center_focus_strong, color: AppTheme.cyanAccent, size: 14),
                            ...
                            child: Row(
                              ...
                              children: [
                                _buildFilterToggle(
                                  ...
                                  onTap: () => setState(() => isClaheEnabled = !isClaheEnabled),
                                ),
                                _buildFilterToggle(
                                  label: "BNN RETICLE",
                                  isActive: isBnnUncertaintyShown,
                                  onTap: () => setState(() => isBnnUncertaintyShown = !isBnnUncertaintyShown),
                                ),
                                _buildFilterToggle(
                                  label: "30S TRAJECTORY",
                                  isActive: isTrajectoryShown,
                                  onTap: () => setState(() => isTrajectoryShown = !isTrajectoryShown),
                                ),
                              ],
                            ),
                          ),
                      ...
                      child: Row(
                        children: specimens.map((sp) {
                          ...
                          return Container(
                            ...
                            decoration: BoxDecoration(
                              color: isSelected ? AppTheme.cyanAccent.withOpacity(0.15) : AppTheme.bgCard,
                              borderRadius: BorderRadius.circular(10),
                              border: Border.all(
                                color: isSelected ? AppTheme.cyanAccent : AppTheme.textMuted.withOpacity(0.3),
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                  ...
                                  style: GoogleFonts.jetBrainsMono(color: AppTheme.goldAccent, fontSize: 10),
                                ),
                              ],
                            ),
                          );
  ...
  Widget _buildFilterToggle({required String label, required bool isActive, required VoidCallback onTap}) {
...
class BoundingBoxOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    for (var sp in specimens) {
      ...
      canvas.drawLine(sp.box.topLeft, Offset(sp.box.left + lineLen, sp.box.top), reticlePaint);
      canvas.drawLine(sp.box.topLeft, Offset(sp.box.left, sp.box.top + lineLen), reticlePaint);
      canvas.drawLine(sp.box.topRight, Offset(sp.box.right - lineLen, sp.box.top), reticlePaint);
      canvas.drawLine(sp.box.topRight, Offset(sp.box.right, sp.box.top + lineLen), reticlePaint);

      // Badge Label
      final String badgeText = "#${sp.trackId} ${sp.speciesName} ${(sp.confidence * 100).toStringAsFixed(0)}% " +
          (showBnnUncertainty ? "BNN:+/-${(sp.bnnEpistemicUncertainty * 100).toStringAsFixed(1)}%" : "");
      ...
      textPainter.paint(canvas, Offset(sp.box.left + 4, sp.box.top - textPainter.height - 4));

      // Speed Badge
      final speedTextSpan = TextSpan(
        text: "v: ${sp.velocityPx.toStringAsFixed(1)}px/s | ${sp.tailBeatFreqHz.toStringAsFixed(1)}Hz",
        style: GoogleFonts.jetBrainsMono(color: const Color(0xFF00F0FF), fontSize: 9),
      );
      final speedPainter = TextPainter(text: speedTextSpan, textDirection: TextDirection.ltr);
      speedPainter.layout();
      ...
      if (showTrajectory && sp.trajectory30sPoints.isNotEmpty) {
        ...
        for (int i = 0; i < sp.trajectory30sPoints.length; i++) {
          ...
          canvas.drawCircle(target, (2 + i * 2.5), Paint()..color = const Color(0xFFF59E0B).withOpacity(0.4));
          current = target;
        }
      }
    }
...
class OceanWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (double x = 0; x < size.width; x += 20) {
      final y = size.height * 0.4 + sin((x / size.width * 4 * pi) + animationValue * 2 * pi) * 15;
      if (x == 0) {
        path.moveTo(x, y);
      } else {
        path.lineTo(x, y);
