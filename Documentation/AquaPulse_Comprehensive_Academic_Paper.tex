...
\newcommand{\urltext}[1]{{\color{linkblue}\underline{\texttt{#1}}}}
\newcommand{\citeblue}[1]{{\color{linkblue}\cite{#1}}}
\newcommand{\refblue}[1]{{\color{linkblue}\ref{#1}}}
...
\urltext{parsabe99@gmail.com}\\[0.05cm]
\urltext{https://aquapulse.parsabe.com}}
...
To overcome these challenges, we present \textbf{AquaPulse}, a multi-stage software architecture and mathematical framework for real-time aquatic vision tracking, stochastic population estimation, and generative intelligence. As detailed in the codebase inventory in Appendix~\refblue{sec:app_codebase} (Table~\refblue{tab:codebase_structure}) and complete directory tree layout in Appendix~\refblue{sec:app_tree}, AquaPulse integrates decoupled processing modules spanning optical preprocessing, neural computer vision, biological census tracking, stochastic Ensemble Kalman Filtering (EnKF), live graphical rendering, Ollama LLM voice dialogue, and multi-format document exporting. To streamline field deployment, production build artifacts are provided as a containerized Linux Docker server image and a standalone Windows setup installer executable that automatically audits system dependencies.
...
This appendix provides supplementary mathematical details for GBIF REST taxonomy integration, information-theoretic diversity indices, the core subsystem architecture directory, and the complete workspace directory tree structure.
...
\subsection{Complete Workspace Directory Tree \& Layout}
\label{sec:app_tree}
The overall physical directory structure of the AquaPulse codebase repository is detailed below:

\begin{verbatim}
AquaPulse/
|-- 0 - Preprocessing/
|   |-- 0 - Source Codes/         # CUDA MSRCR dehazing & LAB CLAHE functions
|   +-- 1 - Insta 360 Process/    # PyTorch 3D ray warping
|-- 1 - Insta 360 Process/        # Secondary wide-angle camera conversion scripts
|-- 3 - AI process/               # Core application runtime codebase
|   |-- models/                   # Prioritized YOLOv8 weight repository (.pt files)
|   |-- assets/                   # UI mascot icons, fonts, and reticle graphics
|   |-- species_snapshots/        # Cropped fish reticle images & wiki_cache/ JSONs
|   |-- session_outputs/          # Timestamped CSV logs, Matplotlib plots, PDF reports
...
|-- 4 - Documentation/            # Academic LaTeX paper sources & BibTeX databases
|-- Dockerfile                    # Server container build specification
|-- AquaPulse_Setup.py            # Windows standalone setup installer wizard
+-- README.md                     # Architecture overview & installation guide
\end{verbatim}
